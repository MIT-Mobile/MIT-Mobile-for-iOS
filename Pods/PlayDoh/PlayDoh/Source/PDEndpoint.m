...
@interface PDEndpoint ()
@property (strong, nonatomic) id slug;
@property (strong, nonatomic) id<PDParameterEncodableType> parameters;
- (NSString *)populatedEndpointUrl;
@end

@implementation PDEndpoint
...
+ (instancetype)endpointWithParameters:(id<PDParameterEncodableType>)parameters {
    return [self endpointWithSlug:nil andParameters:parameters];
}
- (instancetype)initWithParameters:(id<PDParameterEncodableType>)parameters {
    return [self initWithSlug:nil andParameters:parameters];
}

+ (instancetype)endpointWithSlug:(id)slug andParameters:(id<PDParameterEncodableType>)parameters {
    return [[self alloc] initWithSlug:slug andParameters:parameters];
}
- (instancetype)initWithSlug:(id)slug andParameters:(id<PDParameterEncodableType>)parameters {
    self = [self init];
    if (self) {
        self.slug = slug;
...
- (NSDictionary *)slugMappingForClass:(Class)slugClass {
    ...
    for (PDSlugMapping *slugMapping in slugClassMappings) {
...
@implementation PDEndpoint (Networking)
...
- (void)getWithCompletion:(void(^)(id object, NSError *error))completion {
    [PDNetworking getForEndpoint:self withCompletion:completion];
}
- (void)putWithCompletion:(void(^)(id object, NSError *error))completion {
    [PDNetworking putForEndpoint:self withCompletion:completion];
}
- (void)postWithCompletion:(void(^)(id object, NSError *error))completion {
    [PDNetworking postForEndpoint:self withCompletion:completion];
}
- (void)patchWithCompletion:(void(^)(id object, NSError *error))completion {
    [PDNetworking patchForEndpoint:self withCompletion:completion];
}
- (void)deleteWithCompletion:(void(^)(id object, NSError *error))completion {
    [PDNetworking deleteForEndpoint:self withCompletion:completion];
}
...
@implementation PDEndpoint (Slugs)
...
@implementation PDSlugMapping
+ (instancetype)slugMappingWithClass:(Class)classType {
    PDSlugMapping *new = [PDSlugMapping new];
...
@implementation PDEndpoint (HeaderMapping)
