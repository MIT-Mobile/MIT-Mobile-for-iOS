...
@implementation NSObject (JMIntrospection)
...
+ (void)load {
    swiftClassMapping = [self jm_swiftClassMapping];
}
        ...
        if (comps.count == 2 && ![@[@"Foundation", @"Swift"] containsObject:comps.firstObject]) {
            ...
            if ([class conformsToProtocol:@protocol(JSONMappableObject)]) {
                ...
                /*
                 ...
                 mapping["propertyName"~ClassType.self] = "jsonKeyPath" // Swift
                 - or -
                ...
                if (swiftClassMapping[demangledClassName]) {
                    ...
                    NSLog(@"\n JSONMapping won't be able to appropriately introspect these classes and explicit class types must be included in the mapping \n");
                }
